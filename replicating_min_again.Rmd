...
---
# Overview
This script explores the purported relationships between rewiring probability and resoure inflow on a population of cooperating agents' resistance to invasion by a lone defector in an agent-based version of the TLS model (2012; 2017) with network dynamics described by Min et al. (2018). They found that increasing the probability of network rewiring had a positive effect in networks with low average dgree (k = 15), with minimal impact in relatively high degree networks (k = 90) except when close to 1. However, *Min et al. created a dependancy (i.e., negative relationnship) between the probability of strategy updating and probability of network rewiring, as set by the parameter rho*. Here, we examine what happens when the probability of network rewiring is manipulated independently. 
--> Building up to an argument that, in conditions where collective action problems are truly large scale and/or play out with open access resources, our propensity to sever ties with defectors (which is advantageous for dyadic cooperation as well as scenarios where group-level selection dominates) is NOT doing us any favours! 
** but look at function of bridging ties in polycentric governance networks ** 
...
- - > DO I NEED TO MAKE THIS BE BASED ON UTILITY COMPARISONS RATHER THAN STRATEGY AS WELL? 
         ...
         *** to fully recrease Min et al.'s plots, it's a NO *** 

## Errors
- when rho is 0 and c is high, returning NA where true/false needed with `if (strat_i != strat_j) {`

Error in if (agents_own[, .N, strategy][, .N] == 2 && agents_own[strategy ==  : 
  missing value where TRUE/FALSE needed

## CHANGES 
- store network in final step if model reaches equilibrium
...
```{r}
# code from one_run_min used to store networks back when I thought i'd do it every 500 ticks. 
# now decided to record every time a tie changes 
  store_networks <- seq(0, max_t,    # store network data
                          by = 500   # every 500 ticks
  )
  network_list <- vector(        # store info about ties
    mode = "list",               # = as sparse adjacency matrices
    length = 1 + length(store_networks)  # in network_list
  )
  node_list <- data.table(            # strategy in node_list
    id = rep(seq(1, 100, 1), times = 1 + length(store_networks)),
    strategy = NA,
    tick = rep(c(0, 1, seq(500, max_t, 500)), each = N)
  )
```
