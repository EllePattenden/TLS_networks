# OzStar part 1 (pull apart Min et al.)
# generate parameter space for sims 
...
set.seed(31133113)      # swap so each make brain do sense thing and batch 
...
params <- setDT(data.frame(
  ...
  # ******** now overwritten to false in one_run_min.R 
...
# compromised again because I (Elle) want to get on with more interesting stuff 
# = 42840 runs 
...
# stick all parameters in one big data.table and save as test_combo.Rda
...
save(combo, file="test_combo.Rda")
...
combo <- out
save(combo, file = "test_combo_chunked.Rda")

# keep it clean(-ish) 
rm(list = c("replicate", "degree", "resource_inflow", "rho", "combos", "params"))
