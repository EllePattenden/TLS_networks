...
## 04/01/23 - version needed for zoom_tp_d90 is on GIT. Stop overwriting files!
    ...
    if (runif(1, 0, 1) < rho) {        # with probability rho 
      ...
      if (length(which(network[agent_ii[, id], ] == TRUE)) != 0) { # if agent_ii has ties
        ...
        if (comp[agent == "i", strategy == 1] &     # if agent_ii is a cooperator 
            comp[agent == "j", strategy == 0])  {
          
          # NETWORK INTEGRATION MECHANISM 
          options <-  # find agent_ii's cooperative partners who aren't tied to agent_jj 
            setdiff(agents_own[id %in% which(network[comp[agent == "i", id], ] == TRUE) & strategy == 1, id], 
                    agents_own[id %in% which(network[comp[agent == "j", id], ] == TRUE), id])
          
          if (length(options) > 0) { # as long as ii has a cooperative partner to recruit 
            agent_kk <- sample(options, 1)  # pick one randomly; they are agent_kk
            if (length(  # and as long as kk has the potential to join (i.e., more ties than just to ii)
              which(network[agent_kk, ] == TRUE)[which(network[agent_kk, ] == TRUE) !=  
                                                 comp[agent == "i", id]]) > 0 ) { 
              k_to_cull <-         # agent_kk then picks one of their social partners, randomly 
                # we *could* force this to be a cooperator, but it seems reasonable to keep the random random random ... 
                sample(which(network[agent_kk, ] == TRUE)[
                  which(network[agent_kk, ] == TRUE) != comp[agent == "i", id]], 
                  1) 
              # and cuts their tie to them, to maintain <k>, and makes a tie to agent_jj 
              if (length(k_to_cull) > 0) { 
                network[agent_kk, k_to_cull] <- FALSE
                network[k_to_cull, agent_kk] <- FALSE     
                network[agent_kk, comp[agent == "j",id]] <- TRUE
                network[comp[agent == "j",id], agent_kk ] <- TRUE
                # update trackers 
                tie_switched <- 1 
                nodes <- agents_own[, c("id", "strategy")][, tick := tick]
            ...
            tie_switched <- 99999   # record instances where ii == C and jj == D, but ii couldn't get help 
          } 
        } 
