...
# 20/09/22 - back to recording networks for zoom_tp_d90; changed output name
# also added storage of utility and payoff for each agent in node_list 
...
one_run <- function(        # reading in row of values in sequence from combo
  ...
  resource_dynamics <- 
    function(R, c, d, max_R, q, E_total) {     
      ...
      if (new < 0) {
        ...
        # 02.08.22 - see discussion with Floyd about this 
  ...
  node_list <- data.table(
    ...
    utility = NA,                                 # adding in Utility and 
    payoff = NA                                   # payoff for zoom_tp_d90
  ) # will remove rows with strategy == NA at end
  numbers_please <- c("utility", "payoff")        # convert to numeric, 
  node_list[, (numbers_please) := lapply(.SD, as.numeric), 
            .SDcols = numbers_please]
  rm(numbers_please)
  ...
  for (tick in 1:max_t) { 
    ...
    if (agents_own[, .N, strategy][,.N] == 2) {      # only run if both strategies are present 
      ...
      nodes <- agents_own[, c("id", "strategy", "utility", "payoff")][, tick := tick]  # added for ZOOM 
      node_list[nodes, on = c('id', 'tick'), strategy := i.strategy]
      node_list[nodes, on = c('id', 'tick'), utility := i.utility]  # ADDED FOR ZOOM 
      node_list[nodes, on = c('id', 'tick'), payoff := i.payoff]    # MAY NOT WORK! ! ! 
      break 
    }
    ...
    if (length(which(network[agent_i[, id], ] == TRUE)) != 0) { # if agent_i has ties
      ...
      if (comp[agent == "i", strategy] !=   # agent_i and agent_j are playing 
        ...
        if (runif(1, min = 0, max = 1) < utility_diff) {
          ...
          strat_switched <- 1
          p_coop_i <- agents_own[, sum(strategy)] / N
          nodes <- agents_own[, c("id", "strategy", "utility", "payoff")][, tick := tick]
      ...
      if (length(which(network[agent_ii[, id], ] == TRUE)) != 0) { # if agent_ii has ties
        ...
        if (comp[agent == "i", strategy == 1] &     # if agent_ii is a cooperator 
          ...
          if (length(options) != 0 ) { 
            ...
            tie_switched <- 9999999999         # record instances where fully connected coop arises 
          }
  ...
  # FOR zoom_tp_d90
  name <- paste("zoom_tp_d90_replicating_min", "rep", replicate, 
                "degree", degree, 
                "RI", resource_inflow,
                "rho", rho,
                ".Rdata", sep = "_")
